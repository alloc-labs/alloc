...
ray_app = typer.Typer(
    name="ray",
    help="Ray cluster integration.",
    no_args_is_help=True,
)
app.add_typer(ray_app, name="ray")


@ray_app.command("submit")
def ray_submit(
    command: list[str] = typer.Argument(..., help="Training command (after --)"),
    address: str = typer.Option("http://127.0.0.1:8265", "--address", "-a", help="Ray dashboard address"),
    working_dir: Optional[str] = typer.Option(None, "--working-dir", "-w", help="Working directory to upload to Ray workers"),
    upload: bool = typer.Option(False, "--upload", help="Auto-upload artifact to dashboard"),
    full: bool = typer.Option(False, "--full", help="Full monitoring (not calibrate-and-exit)"),
    timeout: int = typer.Option(120, "--timeout", help="Calibration timeout in seconds"),
    no_config: bool = typer.Option(False, "--no-config", help="Skip .alloc.yaml on worker"),
    no_wait: bool = typer.Option(False, "--no-wait", help="Submit and return immediately"),
    pip: Optional[str] = typer.Option(None, "--pip", help="Extra pip packages (comma-separated)"),
    env: Optional[str] = typer.Option(None, "--env", help="Extra env vars (KEY=VAL,KEY2=VAL2)"),
    json_output: bool = typer.Option(False, "--json", help="Output as JSON"),
):
    """Submit a training job to a Ray cluster with GPU monitoring."""
    from alloc.ray_submit import submit_ray_job

    if not command:
        console.print("[red]No command provided.[/red]")
        console.print("Usage: alloc ray submit -- python train.py")
        raise typer.Exit(1)

    # Parse extra pip deps
    extra_pip = [p.strip() for p in pip.split(",") if p.strip()] if pip else None

    # Parse extra env vars
    extra_env = None  # type: Optional[dict]
    if env:
        extra_env = {}
    ...
    if not json_output:
        console.print(f"[green]alloc[/green] [dim]v{__version__}[/dim] — Ray Submit")
        console.print(f"[dim]Cluster:  {address}[/dim]")
        console.print(f"[dim]Command:  {' '.join(command)}[/dim]")
        extra_count = len(extra_pip) if extra_pip else 0
        console.print(f"[dim]Runtime:  alloc[gpu] + {extra_count} extra deps[/dim]")
        console.print()
    ...
    if json_output:
        ...
        if error:
            ...
            color = "green" if status == "SUCCEEDED" else "yellow" if status == "SUBMITTED" else "red"
            console.print(f"Job {job_id} [{color}]{status}[/{color}]")
