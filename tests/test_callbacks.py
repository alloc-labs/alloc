...
# ── Dataloader wait estimation ───────────────────────────────────────────


class TestEstimateDataloaderWait:
    def test_low_cv(self):
        assert _estimate_dataloader_wait(0.05) == 0.0
        assert _estimate_dataloader_wait(0.1) == 0.0

    def test_high_cv(self):
        assert _estimate_dataloader_wait(0.5) == 30.0
        assert _estimate_dataloader_wait(0.8) == 30.0

    def test_mid_cv(self):
        # CV 0.3 → linear interp: (0.3 - 0.1) / 0.4 * 30 = 15.0
        result = _estimate_dataloader_wait(0.3)
...
class TestBuildSidecar:
    def test_basic(self):
        ...
        # Uniform times → CV=0 → 0% wait
        assert data["dataloader_wait_pct"] == 0.0
